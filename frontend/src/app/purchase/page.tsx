...
import {
  ...
  PaymentOption,
  PurchasePageResBody,
} from "@/purchase/types/purchase-response";
...
export default function PurchasePage({
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:8080/api/purchases/checkout?productId=${productId}&quantity=${quantity}`,
    )
      .then((res) => res.json())
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const postcodeStr = (
      form.elements.namedItem("receiver.postcode") as HTMLInputElement
    ).value.trim();
    ...
    const createPurchase = () => {
      const userEmail = (
        form.elements.namedItem("purchaser.email") as HTMLInputElement
      ).value.trim();
      ...
      const body: PurchasePageReqBody = {
        ...
        purchaser: {
          name: (
            form.elements.namedItem("purchaser.name") as HTMLInputElement
          ).value.trim(),
          email: userEmail,
        },
        receiver: {
          name: (
            form.elements.namedItem("receiver.name") as HTMLInputElement
          ).value.trim(),
          phoneNumber: (
            form.elements.namedItem("receiver.phoneNumber") as HTMLInputElement
          ).value.trim(),
          address: (
            form.elements.namedItem("receiver.address") as HTMLInputElement
          ).value.trim(),
          postcode: parseInt(
            (
              form.elements.namedItem("receiver.postcode") as HTMLInputElement
        ...
        paymentOptionId: parseInt(
          (form.elements.namedItem("paymentOptionId") as HTMLInputElement)
            .value,
        ),
      };
