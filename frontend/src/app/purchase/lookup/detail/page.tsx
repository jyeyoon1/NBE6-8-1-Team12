...
export default function PurchaseLookUpResultPage() {
  ...
  const [purchaseItemDetailDto, setPurchaseItemDetailDto] =
    useState<PurchaseItemDetailDto | null>(null);
  const [receiverResDto, setReceiverResDto] = useState<ReceiverResDto | null>(
    null,
  );
    ...
    const fetchPuchaseDetail = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `http://localhost:8080/api/purchases/lookup/detail`,
          {
            method: "POST",
            headers: {
  ...
  if (!purchaseDto || !purchaseItemDetailDto || !receiverResDto)
        ...
        <table className="w-full border rounded-lg overflow-hidden mb-3">
          ...
          <tbody>
            <tr className="text-center text-black">
              ...
              <td className="py-2 px-3">
                {purchaseItemDetailDto.price.toLocaleString()}원
              </td>
